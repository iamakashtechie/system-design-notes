---
title: Ch 6 · Design a Key-Value Store
---
...
<p align="center">
  ![CAP](/images/06-key-value-store/cap.png)
</p>
...
- **CA Systems:** Consistency and Availability while sacrificing partition tolerance.
    ...
    **Since network failure is unavoidable, a distributed system must tolerate network partition. Thus, a CA system cannot exist in real-world applications.**
    ...
    <p align="center">
    ![Server down](/images/06-key-value-store/server-down.png)
    </p>
...
- The N servers are chosen by walking clockwise from the server position and choose the first N servers on the ring to store data copies.Place replicas in distinct data centers to improve reliability in case of virtual nodes.
    ...
    <p align="center">
    ![Data replication](/images/06-key-value-store/data-replication.png)
    </p>
...
- **Quorum Consensus:**
  ...
  - The configuration of W, R and N is a typical tradeoff between latency and consistency. 
    ...
    <p align="center">
    ![Quorum consensus](/images/06-key-value-store/quorum-consensus.png)
    </p>
...
- **Versioning:** 
    ...
    - Versioning means treating each data modification as a new immutable version of data.
        ...
        ![Consisten hashing](/images/06-key-value-store/consistent-server.png)
        ![Inconsistent server](/images/06-key-value-store/inconsistent-server.png)
...
- **Vector Clock**
    ...
    4. **Conflict Resolution:** When conflicts are detected (sibling versions), the system relies on application-specific logic or client intervention to   reconcile the data.
        ...
        <p align="center">
        ![Server hashing](/images/06-key-value-store/vector-clock.png)
        </p>
...
- **Gossip Protocol:**
    ![Gossip protocol](/images/06-key-value-store/gossip-protocol.png)
...
- **Sloppy Quorum:** Use healthy nodes to maintain operations temporarily.
        <p align="center">
        ![Sloppy Quorum](/images/06-key-value-store/sloppy-quorum.png)
        </p>
...
- Working
    ...
    2. **Building a Merkle Tree:**
        - **Step 1:** Divide the key space into buckets.
            ...
            ![Key Bucket](/images/06-key-value-store/key-bucket.png)

        - **Step 2:** Hash each key in a bucket using uniform hashing.

            ![Hash Key Bucket](/images/06-key-value-store/hash-key-bucket.png)

        - **Step 3:** Create a single hash for each bucket.
        
            ![Hash Bucket](/images/06-key-value-store/hash-bucket.png)

        - **Step 4:** Combine hashes of buckets to compute higher-level hashes, culminating in the root hash.

            ![Merkel Tree](/images/06-key-value-store/merkel-tree.png)
...
![Hash Bucket](/images/06-key-value-store/write-path.png)
...
### 2. Read Path
![Hash Bucket](/images/06-key-value-store/read-path.png)
    ![Hash Bucket](/images/06-key-value-store/read-path-without-cache.png)
...
<p align="center">
![Hash Bucket](/images/06-key-value-store/final-architecture.png)
</p>
